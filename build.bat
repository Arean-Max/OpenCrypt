...
REM Step 3: Build InnoSetup installer
echo [3/4] Building installer...
if exist "%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe" (
    cd /d "%ROOT%"
    "%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe" installer\installer.iss >nul
    if exist "%ROOT%\Release\OpenCrypt_Setup_v0.2.0.exe" (
        echo Installer: Release\OpenCrypt_Setup_v0.2.0.exe
    )
) else (
...
echo Installer:  %ROOT%\Release\OpenCrypt_Setup_v0.2.0.exe
